...
const EditMovieForm = (props) => {
  const { setMovies, deleteMovie } = props;
  const { id } = useParams();
  ...
  useEffect(() => {
    const getMovie = async (id) => {
      try {
        const response = await axios.get(`https://nextgen-project.onrender.com/api/s11d3/movies/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`https://nextgen-project.onrender.com/api/s11d3/movies/${id}`, movie);
